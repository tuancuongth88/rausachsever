...
class ControllerStartupSeoUrl extends Controller
{
    public function index()
    {
        ...
        if (isset($this->request->get['_route_'])) {
                    ...
                    if (count($url) > 1) {
                        ...
                        if ($url[0] == 'category_news_id') {
                            if (!isset($this->request->get['cat_news'])) {
                                $this->request->get['cat_news'] = $url[1];
                            } else {
                                $this->request->get['cat_news'] .= '_' . $url[1];
                            }
                        }
            ...
            } elseif (isset($this->request->get['cat_news'])) {
            ...
            if (isset($data['route'])) {
                if (($data['route'] == 'product/product' && $key == 'product_id') || (($data['route'] == 'product/manufacturer/info' || $data['route'] == 'product/product') && $key == 'manufacturer_id') || ($data['route'] == 'information/information' && $key == 'information_id') || ($data['route'] = 'news/news' && $key == 'news_id')) {
                    ...
                    foreach ($cat_newss as $cat_news) {
                        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE `query` = 'category_news_id=" . (int)$cat_news . "'");
